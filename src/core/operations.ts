...
import { searchOperations as upstreamSearchOperations } from './ops/search.ts';
...
// Downstream compatibility: upstream now owns source scoping in ops/search.ts
// via source_id='__all__'. Keep the pre-existing all_sources boolean as a
// thin alias without forking the upstream handler implementation.
const searchOperations: Operation[] = upstreamSearchOperations.map((operation) => {
  if (operation.name !== 'search') return operation;
  const upstreamHandler = operation.handler;
  return {
    ...operation,
    params: {
      ...operation.params,
      all_sources: {
        type: 'boolean',
        description:
          "Span every source for trusted local callers; for remote callers, span only the caller's granted sources.",
      },
    },
    handler: (ctx, params) => upstreamHandler(
      ctx,
      params.all_sources === true ? { ...params, source_id: '__all__' } : params,
    ),
  };
});
