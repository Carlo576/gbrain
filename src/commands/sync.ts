...
async function performSyncInner(engine: BrainEngine, opts: SyncOpts): Promise<SyncResult> {
  ...
  // Preserve repository-local .gbrainignore exclusions after the upstream
  // hoist by enriching the shared strategy/includeHidden option object.
  Object.assign(syncOpts, repoSyncableOptions(repoPath, opts.strategy));
...
async function performFullSync(
  ...
  if (opts.sourceId) {
    ...
    const reconcileSyncOpts = { strategy: opts.strategy ?? 'markdown' } as const;
